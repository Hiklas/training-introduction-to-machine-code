...
\begin{frame}{Babbage Difference Engine}
  \begin{columns}
    ...
    \item
      ...
      \note{Implementation of No.0 and partial No.1}
      \note{Designed No.2 Difference Engine}
      \note{No.0 was completed in 1822}
    \item
      Completely mechanical calculating engine 
    \item
      Superseded in design by analytical engine \cite{AnalyticalEngine}
      \note{Only designed never implemented in Babbage' lifetime}
      \note{General purpose computer design with processor, memory}
      \note{ALU - Arithmetic and Logic unit: tests, branches, loops}
    \item
    \end{itemize}

    \column{0.50\textwidth}
    \begin{center}
      \includegraphics[scale=0.25]{../assets/Difference_Engine_London.eps}
...
\begin{frame}{Bibliography 1}
  ...
  \bibitem[Analytical Engine]{AnalyticalEngine}
    Babbge Analytical Engine: {\url{https://en.wikipedia.org/wiki/Analytical_Engine}}
  \end{thebibliography}
\end{frame}
